...
// function of the anchor and of the volume's extent — both decomposition
// invariants — and never of the buffer in hand. The order is generated by the
// same nested loops in the same sequence, so the reduction is the same
// reduction. `StructuringElement::offsets_at` is where that rule lives.
//
...
// For a box and for both ball rules the two agree — each contains the poles of
// its own bounding box, so the widest member offset *is* the constructor's
// argument. A [stepped](StructuringElement::from_size_stepped) element breaks
// that: a 200-wide axis stepped by two has its last surviving offset at 98, not
// at 99, and an element that declared 99 would fetch a plane no voxel of the
// answer depends on and, worse, would state a dependency it does not have.
// Deriving them from the generated offsets keeps `reach` a *consequence* of the
// element under every parameterisation.
...
/// volume, is every position on that axis. The same argument
/// [`ElementShape::ExtentEllipsoid`] is here for: a parameter space that cannot
/// express another implementation's rule forces a caller matching its numbers to
/// substitute a different filter and call it the same one.
///
...
impl StructuringElement {
    ...
    /// does** — the stride is counted from the clipped start instead
    ...
    /// somewhere, bar the two that refuse such an element outright.
    ///
    ...
    ///   source index, which for those ops is the point. The rule this method
    ///   computes is a property of a *(position, volume)* pair and not of a
    ///   direction of data flow, and both ops write into the same volume their
    ///   points live in, so `volume` is one number on both sides. Each op's
    ///   module header carries the argument in full.
    ///
    ...
    ///
    /// Those two may read [`Self::offsets`] for their members precisely because
    /// they refuse: the only elements they ever see are the ones for which
...
mod tests {
    ...
    /// The anchor of an even window is a choice, and the choice is a shift and
    /// never a rounding: a size of four must span four, not five.
    #[test]
    ...
    /// `the_clipped_starts_reach_is_the_widest_of_its_phases` below and its
