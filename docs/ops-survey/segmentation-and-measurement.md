...
|---|---|---|
| **Global auto-threshold** (Otsu and the rest); any global scalar reduction — also **G1** | ~~Expressible but at N+1 passes: phase 1's whole-lattice fragment reach is also its halo, so a reduction whose answer is one scalar reads the whole volume per block~~ → **expressible at two passes.** `FragmentOp::barrier` states the dependency on the phase below as an ordering, so the halo drops to the reach, and `FragmentOp::reduce` runs the reduction once for the phase rather than once per block | **Built and shipped.** The four fragment-and-join ops are migrated onto it; a scalar reduction is the same declaration with a smaller blob. `docs/design/barriers.md` §10 is the collection, and what a barrier *alone* recovers is a ratio of the fragment set to the volume rather than a constant — measure it per op |
