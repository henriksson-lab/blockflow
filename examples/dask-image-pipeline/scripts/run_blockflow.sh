...
usage() {
  cat <<'EOF'
Usage:
  examples/dask-image-pipeline/scripts/run_blockflow.sh INPUT_DIR OUTPUT_DIR

Runs the Blockflow side of the Dask-image comparison pipeline over every .bmp image
in INPUT_DIR.
...
cargo build -p blockflow-dask-image-pipeline --bin dask-image-pipeline "${cargo_profile_flags[@]}"
profile_dir="debug"
if [[ " ${cargo_profile_flags[*]} " == *" --release "* ]]; then
  profile_dir="release"
fi
blockflow_bin="target/$profile_dir/dask-image-pipeline"
