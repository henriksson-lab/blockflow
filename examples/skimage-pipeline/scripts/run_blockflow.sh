...
usage() {
  cat <<'EOF'
Usage:
  examples/skimage-pipeline/scripts/run_blockflow.sh INPUT_DIR OUTPUT_DIR

Runs the Blockflow side of the scikit-image comparison pipeline over every .bmp image
in INPUT_DIR.
...
cargo build -p blockflow-skimage-pipeline --bin skimage-pipeline "${cargo_profile_flags[@]}"
profile_dir="debug"
