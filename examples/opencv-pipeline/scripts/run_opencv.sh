...
usage() {
  cat <<'EOF'
Usage:
  examples/opencv-pipeline/scripts/run_opencv.sh INPUT_DIR OUTPUT_DIR

Builds and runs the OpenCV C++ reference over every .bmp image in INPUT_DIR.
...
reference_dir="$(cd -- "$script_dir/../reference-opencv" && pwd)"

"$reference_dir/build.sh"
opencv_bin="$reference_dir/build/opencv_pipeline"
