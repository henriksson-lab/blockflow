...
fn the_price_the_sweep_chose_on_is_the_price_the_plan_reports() {
    ...
    let priced = plan
        .iterate_priced(
            ...
            &constraints(model.clone(), vec![0, 1, 2]),
            40,
...
fn price_of(
    ...
    let grid = BlockGrid::new(VOLUME, [64, 64, 64]).expect("a lattice");
    let mut plan = PlanBuilder::new(VOLUME, Dtype::F64, grid);
    let mut only = constraints(model.clone(), axes);
            ...
            for cost in [0.1f64, 1.0, 10.0] {
                ...
                for scale in [2.0f64, 4.0, 8.0, 256.0] {
                    ...
                    let uniform = CostModel {
                        ...
                        // Scaling every coefficient must scale the per-family
                        // corrections by nothing: they are dimensionless
                        // multipliers on the declared cost, so a uniform change
                        // of units leaves them alone. Carried rather than
